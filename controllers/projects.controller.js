  ...
  connection.connect(err => {
    ...
    connection.query(getLocationIDQry, (err, result) => {
      ...
      if (result[0]) {
        ...
        let qry = `insert into projects(title, start_date, capacity, location_id, partner_id, organization_name, img_url, type, project_description) values("${
          data.title
        }", '${data.start_date}', ${data.capacity}, ${data.location_id}, ${
          data.partner_id
        }, "${data.organization_name}", "${data.img_url}", "${data.type}", "${
          ...
          connection.query(addNewLocationQry, (err, result) => {
            ...
            connection.query(getNewLocationIDQry, (err, result) => {
              ...
              let qry = `insert into projects(title, start_date, capacity, location_id, partner_id, organization_name, img_url, type, project_description) values("${
                ...
                data.location_id
              }, ${data.partner_id}, "${data.organization_name}", "${
                data.img_url
...
module.exports.updateProject = (req, res) => {
  ...
  connection.connect(err => {
    ...
    let qry = `UPDATE projects 
      ...
      data.location_id
    }, partner_id=${data.partner_id}, organization_name="${
